<?php
/**
 * jervey Component Analysis Model
 *
 * @version     $Id$
...
class jerveyModelAnalysis extends JModel
{
    ...
    public function getQuestions()
    {
        ...
        ' LEFT JOIN #__jervey_sections sec ON sec.id = ss.section_id'.
        ' LEFT JOIN #__jervey_sections_questions sq ON sec.id = sq.section_id'.
        ' LEFT JOIN #__jervey_questions q ON q.id = sq.question_id'.
        ' LEFT JOIN #__jervey_mat_rows r ON r.question_id = q.id'.
        ' JOIN #__jervey_types t ON q.type_id = t.id'.
    ...
    public function getSessionsForFrequency()
    {
        ...
        if ($row_id != 0)
        {
            ...
            ' FROM #__jervey_users_surveys us'.
            ' JOIN #__jervey_sessions s ON s.affected_id = us.id'.
            ' LEFT JOIN #__jervey_mat_answers mat_ans ON mat_ans.session_id = s.id'.
            ' LEFT JOIN #__jervey_mat_columns col ON col.id = mat_ans.column_id '.
        ...
        else
        {
            ...
            ' FROM #__jervey_users_surveys us'.
            ' JOIN #__jervey_sessions s ON s.affected_id = us.id'.
            ' LEFT JOIN #__jervey_answers ans ON ans.session_id = s.id'.
            ' LEFT JOIN #__jervey_propositions p ON p.id = ans.proposition_id '.
            ' WHERE us.survey_id = '.$survey_id.
            ' ORDER BY s.submit_date';
        }
    ...
    public function getSurveyStats($survey_id)
    {
        $query = ' SELECT s.id, s.title, COUNT(sq.id) AS nbr_questions,'.
                ' (SELECT COUNT(ses.id)'.
                    ' FROM #__jervey_surveys s1'.
                    ' LEFT JOIN #__jervey_users_surveys us ON s1.id = us.survey_id'.
                    ' LEFT JOIN #__jervey_sessions ses ON ses.affected_id = us.id'.
                    ' WHERE s1.id = s.id'.
                    ' ) AS population'.
        ' FROM #__jervey_surveys s'.
        ' LEFT JOIN #__jervey_surveys_sections ss ON s.id = ss.survey_id '.
    ...
    public function getPropositions($question_id)
    {
        ...
        ' FROM #__jervey_questions q'.
        ' JOIN #__jervey_propositions p ON q.id = p.question_id'.
        ' WHERE q.id = '.$question_id.
        ' )'.
        ' UNION'.
        ' ( SELECT c.title AS proposition, c.id AS proposition_id '.
        ' FROM #__jervey_questions q'.
        ' JOIN #__jervey_mat_columns c ON q.id = c.question_id'.
        ' WHERE q.id = '.$question_id.
        ' )'.
    ...
    public function getQuestion($question_id)
    {
        ...
        ' FROM #__jervey_questions q'.
        ' JOIN #__jervey_types t ON t.id = q.type_id'.
        ' WHERE q.id = '.$question_id;
    ...
    public function getNotAnsweredForCrossTab($questionNbr)
    {
        ...
        ' FROM #__jervey_users_surveys us '.
        ' JOIN #__jervey_sessions s ON s.affected_id = us.id '.
        ' JOIN #__jervey_answers ans ON ans.session_id = s.id '.
        ' WHERE us.survey_id = '.$survey_id.
        ' GROUP BY s.id'.
    ...
    public function getAnalysis()
    {
        ...
        ' FROM #__jervey_snapshot_analysis an'.
        ' JOIN #__jervey_analysis_types t ON an.analysis_type_id = t.id'.
        ' JOIN #__jervey_snapshot_survey s ON an.id = s.snapshot_analysis_id';
    ...
    function delete()
    {
        ...
        foreach($cids as $cid)
        {
            ...
            ' FROM #__jervey_snapshot_analysis             AS an'.
            ' LEFT JOIN #__jervey_snapshot_survey          AS s  ON an.id = s.snapshot_analysis_id'.
            ' LEFT JOIN #__jervey_snapshot_question        AS q  ON an.id = q.snapshot_analysis_id'.
            ' LEFT JOIN #__jervey_snapshot_proposition     AS p  ON q.id  = p.snapshot_question_id'.
            ' LEFT JOIN #__jervey_snapshot_cross_tab_value AS v1 ON p.id  = v1.snapshot_proposition1_id'.
            ' LEFT JOIN #__jervey_snapshot_cross_tab_value AS v2 ON p.id  = v2.snapshot_proposition2_id'.
            ' WHERE an.id = ' . $cid;
    ...
    {
        $query = 'SELECT  p.*'.
        ' FROM #__jervey_snapshot_question AS q'.
        ' LEFT JOIN #__jervey_snapshot_proposition AS p  ON q.id  = p.snapshot_question_id'.
