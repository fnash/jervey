<?php
/**
 * jervey Component Data Manager Model
 *
 * @version     $Id$
...
class jerveyModelDataManager extends JModel
{
    ...
    public function getAnswersForExport($survey_id)
    {
        $query =  ' ( SELECT s.id AS survey_id, sess.id AS session_id, sess.submit_date, sess.ip_address, u.name AS user_name, us.user_id,  '.
         ...
         ' LEFT JOIN #__jervey_text_answers ans ON q.id = ans.question_id'.
         ' RIGHT JOIN #__jervey_sessions sess ON sess.id = ans.session_id'.
         ' LEFT JOIN #__jervey_users_surveys us ON sess.affected_id = us.id'.
         ...
         ' LEFT JOIN #__jervey_sections_questions sq ON sec.id = sq.section_id'.
         ' LEFT JOIN #__jervey_questions q ON sq.question_id = q.id'.
         ' LEFT JOIN #__jervey_propositions p ON p.question_id = q.id'.
         ' LEFT JOIN #__jervey_answers ans ON ans.proposition_id = p.id'.
         ' RIGHT JOIN #__jervey_sessions sess ON sess.id = ans.session_id'.
         ' LEFT JOIN #__jervey_users_surveys us ON sess.affected_id = us.id'.
         ' LEFT JOIN #__users u ON us.user_id = u.id'.
         ...
         ' LEFT JOIN #__jervey_mat_answers ans ON ans.row_id = r.id'.
         ' LEFT JOIN #__jervey_mat_columns col ON ans.column_id = col.id'.
         ' RIGHT JOIN #__jervey_sessions sess ON sess.id = ans.session_id'.
         ' LEFT JOIN #__jervey_users_surveys us ON sess.affected_id = us.id'.
         ' LEFT JOIN #__users u ON us.user_id = u.id'.
         ' WHERE q.type_id = 4'.
