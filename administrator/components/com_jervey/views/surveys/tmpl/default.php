...
<table  class="adminlist">
  ...
  <thead>
    ...
    <th width="60"><?php echo JText::_( 'JERVEY_LANG_PUBLISHED');  ?></th>
    <th width="60"><?php echo JText::_( 'JERVEY_LANG_ACCESS');  ?></th>
    <th><?php echo JText::_( 'JERVEY_LANG_SECTIONS');  ?></th>
    <th width="60">
		<?php echo JText::_('JERVEY_LANG_ORDER'); ?>
        <a onclick="submitform();" href="#" >
          <img title="<?php echo JText::_('JERVEY_LANG_SAVE_ORDER'); ?>" 
               alt="<?php echo JText::_('JERVEY_LANG_SAVE_ORDER'); ?>" width="16" height="16"
               src="components/com_jervey/assets/images/saveorder.png">
        </a>
	</th>
    <th width="20">
...
<?php
	if ( count($this->surveys)) :
    	...
    	for ($i = 0, $num=1, $k = 0, $ids =0, $n = count( $this->surveys ); $i < $n; $i++, $ids = $row->survey_id) :
            ...
            $link = JRoute::_( 'index.php?option=com_jervey&controller=surveys&task=edit&cid[]=' . $row->survey_id );
            $linkSection = JRoute::_( 'index.php?option=com_jervey&controller=sections&task=edit&cid[]=' . $row->section_id );

            $linkPublished = JRoute::_( 'index.php?option=com_jervey&controller=surveys&task=setPublished&cid[]=' . $row->survey_id );
            if ($row->published == 0) //non published
            {
                $imgPublished = 'components/com_jervey/assets/images/unpublished.png';
                $titlePublished = JText::_('JERVEY_LANG_PUBLISH');
            }
            else
            {
                $imgPublished = 'components/com_jervey/assets/images/published.png';
                $titlePublished = JText::_('JERVEY_LANG_UNPUBLISH');
            }

            $linkAccess = JRoute::_( 'index.php?option=com_jervey&controller=surveys&task=setAccess&cid[]=' . $row->survey_id );
            if ($row->access_id == 0) // public survey
            {
                $echoAccess = JText::_('JERVEY_LANG_PUBLIC');
                $classAccess = 'green';
            }
            else
            {
                $echoAccess = JText::_('JERVEY_LANG_PRIVATE');
                $classAccess = 'red';
            }
