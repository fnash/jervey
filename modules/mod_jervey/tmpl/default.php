...
<?php
        ...
        $link = JRoute::_('index.php?option=com_jervey&controller=survey&id=' . $publicSurveys[$i]->id) ;
?>
    <li>
...
<?php
        for ($i = 0 ; $i < $nbrMaxSurveys && $i < count($privateSurveys) ; $i++ ) :
            $link = JRoute::_('index.php?option=com_jervey&controller=survey&id=' . $privateSurveys[$i]->id) ;
?>
    <li>
...
?>
<a href="index.php?option=com_jervey&controller=surveys" class="view_all"><?php echo JText::_('VIEW_ALL') ; ?></a>
<?php
        endif ;
	endif ;
?>
